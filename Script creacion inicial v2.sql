...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'pero_compila.estadoReserva'))
    DROP TABLE pero_compila.estadoReserva
...
create table [pero_compila].HabitacionTipo(
habitacionTipo_id int primary key identity,
habitacionTipo_Porcentual numeric(18,2),
HabitacionTipo_Descripcion nvarchar(255),
HabitacionTipo_Codigo numeric(18,0)
)
...
habitacion_hotel int,
PRIMARY KEY (habitacion_numero,habitacion_hotel),
FOREIGN KEY (habitacion_hotel) REFERENCES pero_compila.Hotel(hotel_id),
FOREIGN KEY (habitacion_tipo) references pero_compila.HabitacionTipo(habitacionTipo_id)
)
...
reserva_habitacionhotel int,
reserva_habitacionNro int,
...
FOREIGN KEY (reserva_habitacionNro,reserva_habitacionhotel) REFERENCES pero_compila.Habitacion(habitacion_numero,habitacion_hotel)
)
...
registroEstadia_reserva int not null references [pero_compila].Reserva,
registroEstadia_fechaIn datetime,
registroEstadia_cantDias int,
registroEstadia_usuario int references [pero_compila].Usuario
)
...
INSERT INTO pero_compila.Hotel (hotel_ciudad,hotel_nroCalle,hotel_direccion,hotel_cantEstrellas,
hotel_recargaEstrella)
select distinct Hotel_Ciudad,Hotel_Nro_Calle,Hotel_Calle,Hotel_CantEstrella,Hotel_Recarga_Estrella from gd_esquema.Maestra m
--15 filas


				/*Habitacion Tipo*/
INSERT INTO pero_compila.HabitacionTipo(HabitacionTipo_Codigo,HabitacionTipo_Descripcion,habitacionTipo_Porcentual)
select distinct Habitacion_Tipo_Codigo,Habitacion_Tipo_Descripcion,Habitacion_Tipo_Porcentual from gd_esquema.Maestra
--5 filas

				   /*Habitacion*/
INSERT INTO [pero_compila].[Habitacion]([habitacion_numero],[habitacion_piso],[habitacion_ubicacion],habitacion_hotel,habitacion_tipo)
select distinct Habitacion_Numero,Habitacion_Piso,Habitacion_Frente,h.hotel_id,ht.habitacionTipo_id
from gd_esquema.Maestra m,pero_compila.HabitacionTipo ht , pero_compila.Hotel h where h.hotel_direccion=Hotel_Calle and h.hotel_nroCalle=Hotel_Nro_Calle and ht.HabitacionTipo_Descripcion=m.Habitacion_Tipo_Descripcion
--332 filas
...
values ('Reserva con ingreso (efectivizada)')
				...
				/*Reserva*/
-- AGREGUE LAS HABITACIONES QUE ESTABAN EN NULL, ponemos que  todas las reservas migradas las creo el recepcionista
SET IDENTITY_INSERT pero_compila.Reserva ON
insert into pero_compila.Reserva(reserva_Id,reserva_fechaInicio,reserva_cantDias,reserva_activa,
	reserva_clienteIdentificacion,reserva_clienteMail,reserva_usuario,reserva_estado,reserva_habitacionNro,reserva_habitacionId)
select distinct Reserva_Codigo,Reserva_Fecha_Inicio,Reserva_Cant_Noches,1,Cliente_Pasaporte_Nro,
Cliente_Mail,2,1,m.Habitacion_Numero,ho.hotel_id
from gd_esquema.Maestra m  join pero_compila.Cliente c
on(c.cliente_email=m.Cliente_Mail)
join pero_compila.Habitacion h on(h.habitacion_numero=m.Habitacion_Numero)
join pero_compila.Hotel ho on (ho.hotel_direccion =m.Hotel_Calle and ho.hotel_nroCalle=m.Hotel_Nro_Calle)
order by 1
SET IDENTITY_INSERT pero_compila.Reserva OFF
--96944 filas

				/*Estadia*/
insert into pero_compila.RegistroEstadia(registroEstadia_fechaIn,registroEstadia_cantDias,
	registroEstadia_reserva)
select distinct Estadia_Fecha_Inicio, Estadia_Cant_Noches,r.reserva_Id 
from gd_esquema.Maestra m join pero_compila.Reserva r on(r.reserva_Id=m.Reserva_Codigo)
...
--66 filas
			   /*Consumible*/
insert into pero_compila.Consumible (consumibe_precio,consumible_descripcion,consumible_Codigo)
